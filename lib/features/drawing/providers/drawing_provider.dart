...
import '../drawing_repository_impl.dart';
...
class DrawingNotifier extends StateNotifier<List<Drawing>> {
  ...
  Future<void> updateDrawing(Drawing drawing) async {
    try {
      ...
      result.fold(
        onSuccess: (_) {
          // Optimistic update: Sadece ilgili item'ı güncelle, tüm listeyi yenileme
          final index = state.indexWhere((d) => d.id == drawing.id);
          if (index != -1) {
            state = [
              for (int i = 0; i < state.length; i++)
                if (i == index) drawing else state[i],
            ];
          }
        },
        onFailure: (error) {
