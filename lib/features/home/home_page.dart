...
import '../projects/project_form_page.dart';
...
class HomePage extends StatelessWidget {
  ...
  Widget _buildPortraitButtonFlexible({
    required String text,
    required VoidCallback onTap,
    required double height,
    required double fontSize,
    required double spacing,
    bool isPrimary = false, // 🚨 Yeni parametre
  }) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        boxShadow: [
          BoxShadow(
            color: (isPrimary ? Color(0xFF1565C0) : Colors.grey).withOpacity(
              0.3,
            ),
            blurRadius: 8,
            offset: Offset(0, 4),
      ...
      child: ElevatedButton(
        ...
        style: ElevatedButton.styleFrom(
          backgroundColor: isPrimary ? Colors.transparent : Colors.grey[300],
          foregroundColor: isPrimary ? Colors.white : Color(0xFF1565C0),
          elevation: 0, // Gölge Container'dan geliyor
  ...
  Widget _buildPortraitCloseButtonFlexible({
    required BuildContext context,
    required double height,
    required double fontSize,
    required double spacing,
    required bool isSmallPhone, // 🚨 YENİ: Küçük ekran parametresi
  }) {
    // 🚨 YENİ: Çok küçük ekranlarda sadece ikon göster
    final bool showOnlyIcon = isSmallPhone && height < 50;
    ...
    return SizedBox(
      ...
      child: OutlinedButton(
        ...
        style: OutlinedButton.styleFrom(
          ...
          side: BorderSide(color: Colors.grey[400]!),
          padding: EdgeInsets.symmetric(
            horizontal: showOnlyIcon ? 0 : height * 0.3,
          ),
          minimumSize: Size.zero,
          tapTargetSize: MaterialTapTargetSize.shrinkWrap,
        ),
        child: showOnlyIcon
            ? Icon(
                Icons.exit_to_app_rounded,
                size: height * 0.5, // Yüksekliğin %50'si
                color: Colors.grey[600],
              )
            : Row(
                mainAxisAlignment: MainAxisAlignment.center,
  ...
  Widget _buildLandscapeButton({
    required String text,
    required VoidCallback onTap,
    bool isNarrow = false, // 🚨 YENİ: Dar ekran parametresi
  }) {
    // 🚨 YENİ: Dar ekranlarda daha kompakt boyutlar
    final double minHeight = isNarrow ? 44 : 60; // 60 -> 44 (dar ekran)
    final double maxHeight = isNarrow ? 52 : 70;
    final double minWidth = isNarrow ? 160 : 200;
    final double maxWidth = isNarrow ? 280 : 400;
    final double fontSize = isNarrow ? 13 : 16; // 🚨 YENİ: Küçük yazı
    final double padding = isNarrow ? 10 : 16; // 🚨 YENİ: Az padding
  ...
  // 🚨 YENİ: Yatay mod çıkış butonu
  // 🚨 YENİ: Parametreli versiyon
  Widget _buildLandscapeCloseButton(
    BuildContext context, {
    bool isNarrow = false,
  }) {
    final double minHeight = isNarrow ? 38 : 50;
    final double maxHeight = isNarrow ? 46 : 60;
    final double minWidth = isNarrow ? 160 : 200;
    final double maxWidth = isNarrow ? 280 : 400;
    final double fontSize = isNarrow ? 12 : 16;
    ...
    return ConstrainedBox(
      ...
      child: OutlinedButton.icon(
        ...
        style: OutlinedButton.styleFrom(
          ...
          maximumSize: Size(double.infinity, maxHeight),
          shape: RoundedRectangleBorder(
            borderRadius: BorderRadius.circular(isNarrow ? 10 : 15),
          ),
          side: BorderSide(color: Colors.grey[400]!),
  ...
  Widget _buildPortraitCloseButton(
    BuildContext context,
    double scaleFactor, {
    double? height,
  }) {
    final double actualHeight = (height ?? 62) * scaleFactor;

    // 🚨 YENİ: Çok küçük ekranlarda sadece ikon göster
    final bool showOnlyIcon = scaleFactor < 0.8;
    final double fontSize =
        (height != null && height < 50 ? 12 : 14) *
        scaleFactor; // 🚨 DEĞİŞTİ: Daha küçük
    ...
    return SizedBox(
      ...
      child: OutlinedButton.icon(
        ...
        icon: Icon(
          ...
          size: showOnlyIcon
              ? 24 * scaleFactor
              : 16 * scaleFactor, // 🚨 DEĞİŞTİ
          color: Colors.grey[600],
        ),
        label: showOnlyIcon
            ? const SizedBox.shrink() // 🚨 EKLE: Sadece ikon göster
            : Text(
                'UYGULAMADAN ÇIK',
                style: TextStyle(
                  fontSize: fontSize,
        ...
        style: OutlinedButton.styleFrom(
          ...
          tapTargetSize: MaterialTapTargetSize.shrinkWrap,
          padding: EdgeInsets.symmetric(
            horizontal: showOnlyIcon
                ? 0
                : (8 * scaleFactor), // 🚨 DEĞİŞTİ: Dar ekranda az padding
          ),
        ),
      ),
    );
